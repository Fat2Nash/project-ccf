...
<head>
    ...
    <title>Thiriot-Location | Locations</title>
                                        ...
                                        <tr>
                                            @foreach ($locations as $location)
                                                ...
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div>
                                                        <h4 class="text-gray-700 ">{{ $location->adresse }}</h4>
                                                        <p class="text-gray-500 ">{{ $location->ville }},
                                                            {{ $location->code_postal }}, {{ $location->pays }}</p>
                                                    </div>
                                                </td>
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div>
                                                        <h4 class="text-gray-700 ">
                                                            {{ $clients->firstWhere('id_client', $location->client_id)->nom }}
                                                            {{ $clients->firstWhere('id_client', $location->client_id)->prenom }}
                                                        </h4>
                                                ...
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div>
                                                        <p class="text-gray-700 ">
                                                            @php
                                                                $seconds = $location->Temps_fonct; // Exemple de temps en secondes

                                                                // Calcul des heures
                                                                $hours = floor($seconds / 3600);

                                                                // Calcul des minutes restantes après avoir enlevé les heures
                                                                $minutes = floor(($seconds % 3600) / 60);

                                                                echo $hours . ' : ' . $minutes;
                                                            @endphp</p>
                                                        <p class="text-gray-500 ">{{ $location->Louer_le }} -
                                                            {{ $location->Rendu_le }}</p>
                                                    </div>
                                                </td>
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                        ...
                        <div x-show="isOpen"
                            ...
                            <form action="/nouvelle_location" method="post">
                                ...
                                <div class="relative flex items-center py-5">
                                    ...
                                    <span class="flex-shrink mx-4 text-gray-600">Fiche location</span>
                                    <div class="flex-grow border-t border-gray-600"></div>
                                </div>
                                <div class="container items-center p-2">
                                    <h1 class="font-bold text-center">Client</h1>
                                    ...
                                    <label>Adresse : </label><input name="adresse" type="text"
                                        placeholder="11 Rue de Mirecourt">
                                    <label>Ville : </label><input name="ville" type="text"
                                        placeholder="Ville-sur-Illon">
                                    <label>Code postal : </label><input name="code_postal" type="text"
                                        placeholder="88270">
                                    <label>Pays : </label><input name="pays" type="text" placeholder="France">
                                </div>
                                <h1 class="font-bold text-center">Engin</h1>

                                <select name="id_engin" type="select"
                                        placeholder="Pelle">
                                        @foreach ($engins as $engin)
                                            <option value="{{ $engin->id_engins }}">{{ $engin-> marque }}
                                                {{ $engin->modele }} ({{ $engin -> id_engins}})</option>
                                        @endforeach
                                    </select>


                                    <label>Date d'emprunt </label><input name="emprunt" type="datetime-local"
                                        >
                                    <label>Date de retour prévue : </label><input name="retour" type="datetime-local"
                                    >
                            ...
                            <button @click="isOpen = false"
                                class="flex justify-center w-1/2 px-5 py-2 text-sm text-white transition-colors duration-200 bg-red-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-red-600">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-5 h-5 rotate-45">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                Annuler</button>
